...
class Routing(nn.Module):
    ...
    def forward(self, low_capsule, seq_len):
        ...
        for i in range(self.iteration): # 迭代次数r
            ...
            low_capsule_new = torch.einsum('ijk,lo->ijo', (low_capsule, self.S_matrix))
...
class MIND(BasicModel):
    """MIND模型主网络，中间包括了胶囊网络"""
    def __init__(self, feature_size, softmax_dims, embedding_dim=8, max_len=4, input_units=4, output_units=8, iteration=1, max_k=1, p=2):
        """
        ...
        self.capsule_linear = nn.Linear(self.embedding_dim, output_units)
        # 两个relu层
        ...
        self.final_linear = nn.Linear(8, len(shop_id_dict) + 1)
    def forward(self, cat_fea_sex, cat_fea_level_id, iter_fea_shop_id, iter_fea_cate, iter_fea_floor, candidate_shop_id, candidate_cate):
        ...
        embedding_candidate_cate = self.embed_cate(candidate_cate)
        # label embed
        candidate_feature = sum([embedding_candidate_cate, embedding_candidate_shop_id])
        
        # print("candidate_feature", candidate_feature)
        user_ids_embedding = self.user_linear(user_other_feature) # batchsize * 1 * output_units
        label_embedding = self.label_linear(candidate_feature) # batchsize *  output_units
        

        capsule_output = self.routing(seq_embed, seq_lens) # seq_lens没有定义
        capsule_output = self.capsule_linear(capsule_output) # batch_size * max_k * output_units
        ...
        pos_label_embedding = label_embedding.unsqueeze(dim=1)
        ...
        # 将nn.linear放到forward中就每一次都会初始化一次。。。
...
class GRU4REC(BasicModel):
    ...
    def fit(self, train_loader, valid_loader, train_epoch):
        device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
