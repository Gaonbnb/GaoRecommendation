...
class PrepareData:
    ...
    def train_sample_data(self):
        ...
        self.sample_data = self.sample_data.sample(frac=1.0)
...
class JdataDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # else:
        #     cur_data[3] = list(map(int, cur_data[2].strip("[]").strip(","))) if "," in cur_data[3] else list(map(int, cur_data[2].strip("[]")))
        ...
        # else:
        #     cur_data[4] = list(map(int, cur_data[2].strip("[]").strip(","))) if "," in cur_data[4] else list(map(int, cur_data[2].strip("[]")))
        
        cur_data[2] = [*map(int, cur_data[2].split("#"))] if isinstance(cur_data[2], str) else cur_data[2]
        cur_data[3] = [*map(int, cur_data[3].split("#"))] if isinstance(cur_data[3], str) else cur_data[3]
        cur_data[4] = [*map(int, cur_data[4].split("#"))] if isinstance(cur_data[4], str) else cur_data[4]
...
def collate_fn(batch_data):
    ...
    batch_shop_id = torch.nn.utils.rnn.pad_sequence(batch_shop_id, batch_first=True, padding_value=0)[:,:16]
    batch_cate = torch.nn.utils.rnn.pad_sequence(batch_cate, batch_first=True, padding_value=0)[:,:16]
    batch_floor = torch.nn.utils.rnn.pad_sequence(batch_floor, batch_first=True, padding_value=0)[:,:16]
    

    batch_sex = torch.tensor([item.item() for item in batch_sex])

    batch_level_id = torch.tensor([item.item() for item in batch_level_id])
    
    batch_sex = batch_sex + 1

    batch_candidate_shop_id = torch.tensor([item.item() for item in batch_candidate_shop_id])
    

    batch_candidate_cate = torch.LongTensor([item.item() for item in batch_candidate_cate])
...
if __name__ == "__main__":
    ...
    # 这里数据是正确的
    train_loader = DataLoader(train_data, batch_size=512, shuffle=True, collate_fn=collate_fn, drop_last=True)
