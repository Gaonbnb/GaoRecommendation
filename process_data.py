...
def generate_dict(colname):
    ...
    # 从1开始计数
    shop_id_dict = dict(zip(unq, idx + 1))
    return shop_id_dict
...
if __name__ == "__main__":
    ...
    neg_dict_df = pd.read_csv("neg_sample_df.csv", index_col=0)
    # 控制负采样数量
    neg_dict_df = neg_dict_df.sample(n=150000, replace=False, random_state=1, axis=0)
    neg_dict_df = neg_dict_df.reset_index(drop=True)
    # 用户对应历史shop字典
    ...
    # 为模型输入做准备，存储时list整体变成str的问题
    sample_data.shop_id = sample_data.shop_id.map(lambda x : "#".join("%s" % id for id in x))
    sample_data.cate = sample_data.cate.map(lambda x: "#".join("%s" % id for id in x))
    sample_data.floor = sample_data.floor.map(lambda x: "#".join("%s" % id for id in x))
    ...
    save_dict(cate_dict, "cate_dict")
    save_dict(floor_dict, "floor_dict")
