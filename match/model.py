...
class DSSM(BasicModel):
    """DSSM模型"""
    def __init__(self, feature_size, embedding_dim = 4, hidden_dims = [256, 128, 64], dropout = 0.5):
        """
        ...
        self.embed_shop_id = nn.Embedding(self.feature_size[2], self.embedding_dim)
        self.embed_cate = nn.Embedding(self.feature_size[3], self.embedding_dim)
        self.embed_floor = nn.Embedding(self.feature_size[4], self.embedding_dim)
    ...
    def fit(self, train_loader, valid_loader, train_epoch = 5, learning_rate = 0.005, weight_decay = 0.001):
        """模型进行训练"""
        best_auc = 0.0
        # 选择设备
...
class YouTubeDNN(BasicModel):
    """YouTubeDNN模型"""
    def __init__(self, feature_size, softmax_dims, embedding_dim = 4, hidden_dims = [128, 64, 32], dropout = 0.5):
        """
        输出softmax的结果
        """
    ...
    def forward(self, cat_fea_sex, cat_fea_level_id, iter_fea_shop_id, iter_fea_cate, iter_fea_floor, candidate_shop_id, candidate_cate):
        ...
        self.x = self.relu_layer(embedding_cat)
        # 将之前的多分类问题利用负采样变成二分类问题
        
        score = self.output(self.x)
    ...
    def predict(self, embedding_loader, get_user_embedding=False, get_item_embedding=False):
        ...
        if get_item_embedding:
            # output表
            return self.output.weight.data
        else:
            with torch.no_grad():
                all_user_embedding = []
                ...
                for step, (x, label) in enumerate(embedding_loader):
                    ...
                    all_user_embedding.extend(self.x.numpy().tolist())
